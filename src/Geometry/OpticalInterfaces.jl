...
# could be reworked to include translucent as well I guess?
...
struct FresnelInterface{T} <: OpticalInterface{T}
    ...
    function FresnelInterface{T}(
        ...
        Z <: AbstractTransparentMaterial,
        Y <: AbstractTransparentMaterial,
        T <: Real,
    }
...
function Base.show(io::IO, a::FresnelInterface{R}) where {R <: Real}
    return print(
        io,
        "FresnelInterface($(glassname(a.insidematerial)), $(glassname(a.outsidematerial)), $(a.reflectance), $(a.transmission), $(a.interfacemode))",
    )
end
...
function transmissiveinterface(
    ::Type{T},
    insidematerial::X,
    outsidematerial::Y,
) where {
    T <: Real,
    X <: AbstractTransparentMaterial,
    Y <: AbstractTransparentMaterial,
}
    return FresnelInterface{T}(
        insidematerial,
...
function opaqueinterface(::Type{T} = Float64) where {T <: Real}
    return FresnelInterface{T}(
        Air,
        Air,
        reflectance = zero(T),
...
AllOpticalInterfaces{T} = Union{NullInterface{T}, FresnelInterface{T}}
...
# TODO REMOVE
# Sam 2022-02-24 don't need this for julia-1.6+
