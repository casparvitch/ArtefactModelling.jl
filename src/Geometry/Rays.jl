...
@from "Transforms.jl" import Transforms: Transform, rotate

# export AbstractRay, Ray, origin, direction, closestpointonray
...
struct Ray{T, N} <: AbstractRay{T, N}
    ...
    function Ray(
        origin::AbstractArray{T, 1},
        direction::AbstractArray{T, 1},
    ) where {T <: Real}
        @assert length(origin) == length(direction)
        N = length(origin)
        return new{T, N}(
            SVector{N, T}(origin),
            normalize(SVector{N, T}(direction)),
        )
    end
end
...
function Base.print(io::IO, a::Ray{T, N}) where {T, N}
    ...
    return println(io, "$(rpad("Direction:", 20)) $(direction(a))")
end
...
"""
function α(ray::AbstractRay{T, N}, point::SVector{N, T}) where {T <: Real, N}
    return dot(direction(ray), (point .- origin(ray)))
end
