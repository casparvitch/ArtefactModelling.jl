...
struct BezierCurve{P, S, N, M} <: Spline{P, S, N, M} # this has a funny name because of name conflict with Plots.
    ...
    function BezierCurve{P, S, N, M}(
        controlpoints::AbstractArray{<:AbstractArray{S, 1}, 1},
    ) where {P <: CurveType, S <: Number, N, M}
        @assert length(controlpoints) == M + 1 && length(controlpoints[1]) == N
        return new{P, S, N, M}([
            SVector{N, S}(point) for point in controlpoints
        ])
    end
end
    ...
    for spatialindex in 0:(N - 1)
        ...
        for j in 0:M
            for k in 0:movinglineorder
                ...
                result[row, colblock + k] =
                    Bc(j, M) *
                    Bc(k, movinglineorder) *
                    ctrlpts[j + 1][spatialindex + 1]
            end
        end
    end
    return result
...
struct BezierSurface{P, S, N, M} <: SplineSurface{P, S, N, M}
    ...
    function BezierSurface{P, S, N, M}(
        controlpoints::AbstractArray{<:AbstractArray{S, 1}, 2},
    ) where {P <: CurveType, S <: Real, N, M}
        @assert (M + 1, M + 1) == size(controlpoints) &&
                length(controlpoints[1, 1]) == N
...
function deepcopy(a::BezierSurface{P, S, N, M}) where {P, S, N, M}
    return BezierSurface{P, S, N, M}(a.controlpolygon)
end
function uvrange(::Type{BezierSurface{P, S}}) where {P, S <: Real}
    return ((zero(S), one(S)), (zero(S), one(S)))
end
function uvrange(::BezierSurface{P, S}) where {P, S <: Real}
    return uvrange(BezierSurface{P, S})
end
function onsurface(
    ::BezierSurface{P, S, 3, M},
    ::SVector{3, S},
) where {P, S <: Real, M}
    return false
end
...
# DECASTELJAU
@inline function decasteljau(
    controlpoints::AbstractArray{SVector{N, S}, 1},
    ::Val{2},
    u::T,
) where {T <: Real, S, N}
    w = 1 - u
    return (controlpoints[1] * w + controlpoints[2] * u) * w +
           (controlpoints[2] * w + controlpoints[3] * u) * u
end
...
function point(
    surf::BezierSurface{P, S, N, M},
    u::T,
    v::T,
) where {T <: Real, P, S, N, M}
...
@inline function derivative(
    controlpoints::SVector{4, SVector{N, S}},
    u::T,
) where {T <: Real, N, S}
...
@inline function derivative(
    controlpoints::SVector{5, SVector{N, S}},
    u::T,
) where {T <: Real, N, S}
    d1 = controlpoints[2] .- controlpoints[1]
    d2 = controlpoints[3] .- controlpoints[2]
...
@inline function derivative(
    controlpoints::MVector{Q, SVector{N, S}},
    u::T,
) where {T <: Real, N, S, Q}
...
# PARTIALS
function partials(
    surf::BezierSurface{Euclidean, S, N, 3},
    u::T,
    v::T,
) where {T <: Real, S, N}
    q1 = decasteljau(view(surf.controlpolygon, :, 1), Val(3), u)
...
function partials(
    surf::BezierSurface{Euclidean, S, N, 4},
    u::T,
    v::T,
) where {T <: Real, S, N}
    q1 = decasteljau(view(surf.controlpolygon, :, 1), Val(4), u)
...
function partials(
    surf::BezierSurface{Euclidean, S, N, M},
    u::T,
    v::T,
) where {T <: Real, S, N, M}
...
function partials(
    surf::BezierSurface{Rational, S, N, M},
    u::T,
    v::T,
) where {T <: Real, S, N, M}
...
function euclideanpoint(
    curve::BezierCurve{Euclidean, S, N, M},
    u::T,
) where {T <: Real, S, N, M}
    return point(curve, u)
end

function euclideanpoint(
    curve::BezierCurve{Rational, S, N, M},
    u::T,
) where {T <: Real, S, N, M}
    temp = point(curve, u)
...
function inside(
    surf::AcceleratedParametricSurface{S, 3, BezierSurface{P, S, 3, M}},
    p::SVector{3, S},
) where {P, S <: Real, M}
