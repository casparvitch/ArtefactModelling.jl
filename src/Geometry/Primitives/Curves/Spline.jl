...
function toeuclidean(point)
    return SVector{length(point) - 1}([
        point[i] / point[end] for i in 1:(lastindex(point) - 1)
    ])
end

function euclideancontrolpoints(
    curve::Spline{Rational, S, N, M},
) where {S, N, M}
    return toeuclidean.(curve.controlpolygon)
end

function euclideancontrolpoints(
    curve::Spline{Euclidean, S, N, M},
) where {S, N, M}
