...
function findspan(curve::BSplineCurve{T, S, N, M}, u) where {T, S, N, M}
    return findspan(curve.knotvector, M, u)
end
...
function point(
    curve::BSplineCurve{P, S, N, M},
    u::T,
)::SVector{N, S} where {T <: Real, P, S, N, M}
...
function euclideanpoint(
    curve::BSplineCurve{Euclidean, S, N, M},
    ::T,
)::SVector where {T <: Real, S, N, M}
...
function euclideanpoint(
    curve::BSplineCurve{Rational, S, N, M},
    u::T,
)::SVector where {T <: Real, S, N, M}
...
struct BSplineSurface{P, S, N, M} <: SplineSurface{P, S, N, M}
    ...
    function BSplineSurface{P, S, N, M}(
        knots::KnotVector{S},
        controlpoints::AbstractArray{<:AbstractArray{S, 1}, 2},
    ) where {P, S, N, M}
    ...
    function BSplineSurface{P, S, N, M}(
        ...
        newpoints = [MVector{N, S}(point) for point in controlpoints]
        return new{P, S, N, M}(uknots, vknots, newpoints)
    end
end

function numknots(surf::BSplineSurface)
    return (numknots(surf.uknotvector), numknots(surf.vknotvector))
end

function uvrange(surface::BSplineSurface)
    return (urange(surface.uknotvector), urange(surface.vknotvector))
end

function findspan(surface::BSplineSurface{T, S, N, M}, u, v) where {T, S, N, M}
    return (
        findspan(surface.uknotvector, M, u),
        findspan(surface.vknotvector, M, v),
    )
end

function insertknot(
    knots::AbstractArray{S, 1},
    knotindex::Int,
    controlpoints::Array{MVector{N, S}, 1},
    curveorder::Int,
) where {N, S}
    ...
    newcontrolpoints = vcat(
        ...
        K,
        controlpoints[(knotindex + 1):end],
    )

    for i in (knotindex - M + 1):knotindex
        αᵢ = (knotvalue - knots[i]) / (knots[i + M] - knots[i])
        @. newcontrolpoints[i] =
            αᵢ * controlpoints[i] + (1 - αᵢ) * controlpoints[i - 1]
    end
...
function insertknot(
    curve::BSplineCurve{T, S, N, M},
    knotindex::Int,
) where {T, S, N, M}
    newknots, newcontrolpoints =
        insertknot(curve.knotvector.knots, knotindex, curve.controlpolygon, M)
...
function insertknots(curve::BSplineCurve{T, S, N, M}) where {T, S, N, M}
    newknots, newcontrolpoints =
        insertknots(curve.knotvector.knots, curve.controlpolygon, M)
    return BSplineCurve{T, S, N, M}(newknots, newcontrolpoints)
end

function insertknots(surf::BSplineSurface{T, S, N, M}) where {T, S, N, M}
    # TODO avoid using '
    newvknots, temppoints =
        expandcontrolpoints(surf.vknotvector.knots, surf.controlpolygon, M)
    newuknots, temppoints =
        expandcontrolpoints(surf.uknotvector.knots, copy(temppoints'), M)
    ...
    return BSplineSurface{T, S, N, M}(
        KnotVector{S}(newuknots),
        KnotVector{S}(newvknots),
        copy(temppoints'),
    )
end

function expandcontrolpoints(
    vknots::AbstractArray{S, 1},
    controlpolygon::Array{MVector{N, S}, 2},
    curveorder::Int,
) where {N, S}
    ...
    let newknots = vknots
        for i in 1:upts
            newknots, newcontrolpoints =
                insertknots(vknots, view(controlpolygon, i, :), curveorder)
...
function insertknots(
    ...
    for insertion in insertionknots
        for i in 1:insertion[2]
            newknots, newcontrolpoints = insertknot(
                newknots,
                insertion[1] + offset,
                newcontrolpoints,
                curveorder,
...
function tobeziersegments(surf::BSplineSurface{P, S, N, M}) where {P, S, N, M}
    ...
    # println("numsegments $numsegments")
    segments =
        Array{BezierSurface{P, S, N, M}}(undef, numusegments, numvsegments)
    # println("usges $numusegments vseg $numvsegments")
    for i in 1:numusegments
        for j in 1:numvsegments
            startu = (i - 1) * M + 1
            startv = (j - 1) * M + 1
            segments[i, j] = BezierSurface{P, S, N, M}(
                newcontrolpoints[startu:(startu + M), startv:(startv + M)],
            )
        end
    end
...
function point(
    surface::BSplineSurface{P, S, N, M},
    u::T,
    v::T,
)::SVector{N, S} where {T <: Real, P, S, N, M}
    ...
    for j in 1:(M + 1)
        for i in 1:(M + 1)
            pt =
                surface.controlpolygon[spanu - (M + 1) + i, spanv - (M + 1) + j]
            c .= c .+ ubases[i] .* pt
        end
        sum .= sum .+ vbases[j] .* c
        c .= 0
