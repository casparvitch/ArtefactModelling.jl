...
module Chebyshev
using FromFile: @from
@from "../../utilities.jl" import NaNsafeacos
...
struct ChebyshevSurface{T, N, P} <: ParametricSurface{T, N}
    ...
    function ChebyshevSurface(
        halfsizeu::T,
        halfsizev::T,
        chebycoeff::Union{Nothing, Vector{Tuple{Int, Int, T}}};
        radius::T = typemax(T),
        conic::T = zero(T),
    ) where {T <: Real}
        @assert !isnan(halfsizeu) &&
                !isnan(halfsizev) &&
                !isnan(radius) &&
                !isnan(conic)
        ...
        @assert one(T) -
                (1 / radius)^2 *
                (conic + one(T)) *
                (halfsizeu^2 + halfsizev^2) > 0 "Invalid surface (conic/radius combination: $radius, $conic)"
        offset = zero(T)
        ...
        return new{T, 3, P}(
            ...
            conic,
            bounding_prism,
            SVector{P, Tuple{Int, Int, T}}(P === 0 ? [] : chebycoeff),
            offset,
        )
    end
end

function uvrange(::Type{ChebyshevSurface{T, N, P}}) where {T <: Real, N, P}
...
function point(
    s::ChebyshevSurface{T, 3, P},
    u::T,
    v::T,
)::SVector{3, T} where {T <: Real, P}
    x = u * s.halfsizeu
    y = v * s.halfsizev
    r2 = (x^2 + y^2)
...
function partials(
    ...
    u::T,
    v::T,
)::Tuple{SVector{3, T}, SVector{3, T}} where {T <: Real, P}
    x = u * s.halfsizeu
    y = v * s.halfsizev
    r2 = x^2 + y^2
    ...
    return SVector{3, T}(s.halfsizeu, 0.0, dhdu),
    SVector{3, T}(0.0, s.halfsizev, dhdv)
end
...
function surfaceintersection(
    surf::AcceleratedParametricSurface{T, 3, ChebyshevSurface{T, 3, P}},
    r::AbstractRay{T, 3},
) where {T <: Real, P}
...
function BoundingBox(surf::ChebyshevSurface{T, 3, P}) where {T <: Real, P}
    ...
    q =
        ...
        (one(T) + surf.conic) *
        surf.curvature^2 *
        (surf.halfsizeu^2 + surf.halfsizev^2)
    if q < zero(T)
        throw(
            ErrorException(
    ...
    hmax =
        surf.curvature * (surf.halfsizeu^2 + surf.halfsizev^2) /
        (one(T) + sqrt(q))
    if hmax > zero(T)
        zmax += hmax
    else
