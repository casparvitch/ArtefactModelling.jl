...
function uvrange(::Type{Sphere{T, N}}) where {T <: Real, N}
    return ((-T(π), T(π)), (zero(T), T(π)))
end

function onsurface(sph::Sphere{T, 3}, x::T, y::T, z::T) where {T <: Real}
    return samepoint(x^2 + y^2 + z^2, radius(sph)^2)
end

function inside(sph::Sphere{T, 3}, x::T, y::T, z::T) where {T <: Real}
    return x^2 + y^2 + z^2 - radius(sph)^2 < zero(T)
end

function point(sph::Sphere{T, 3}, ϕ::T, θ::T) where {T <: Real}
    return SVector{3, T}(
        radius(sph) * sin(θ) * cos(-ϕ),
        radius(sph) * sin(θ) * sin(-ϕ),
        radius(sph) * cos(θ),
    )
end

function normal(::Sphere{T, 3}, ϕ::T, θ::T) where {T <: Real}
    return SVector{3, T}(sin(θ) * cos(-ϕ), sin(θ) * sin(-ϕ), cos(θ))
end

function partials(sph::Sphere{T, 3}, ϕ::T, θ::T) where {T <: Real}
    return SVector{3, T}(
        radius(sph) * sin(θ) * -sin(-ϕ),
        radius(sph) * sin(θ) * cos(-ϕ),
        0.0,
    ...
    SVector{3, T}(
        radius(sph) * cos(θ) * cos(-ϕ),
        radius(sph) * cos(θ) * sin(-ϕ),
        radius(sph) * -sin(θ),
    )
end
...
function BoundingBox(sph::Sphere{T, 3}) where {T <: Real}
    return BoundingBox(
        -radius(sph),
        radius(sph),
        -radius(sph),
