...
struct Ellipse{T} <: PlanarShape{T}
    ...
    function Ellipse(
        ...
        return new{T}(
            Plane(n̂, centrepoint, interface = interface),
            halfsizeu,
            halfsizev,
            uvec,
            vvec,
...
function Base.show(io::IO, a::Ellipse{T}) where {T <: Real}
    return print(
        io,
        "Ellipse{$T}($(centroid(a)), $(normal(a)), $(a.halfsizeu), $(a.halfsizev), $(interface(a)))",
    )
end

function uvrange(::Type{Ellipse{T}}) where {T <: Real}
    return ((-T(π), T(π)), (zero(T), one(T)))
end # θ and ρ

function point(r::Ellipse{T}, θ::T, ρ::T) where {T <: Real}
    return centroid(r) +
           ρ * (r.halfsizeu * cos(θ) * r.uvec + r.halfsizev * sin(θ) * r.vvec)
end
function partials(r::Ellipse{T}, θ::T, ρ::T) where {T <: Real}
    return (
        ρ * (r.halfsizeu * -sin(θ) * r.uvec + r.halfsizev * cos(θ) * r.vvec),
        (r.halfsizeu * cos(θ) * r.uvec + r.halfsizev * sin(θ) * r.vvec),
    )
end

function uv(r::Ellipse{T}, x::T, y::T, z::T) where {T <: Real}
    return uv(r, SVector{3, T}(x, y, z))
end
...
function onsurface(a::Ellipse{T}, point::SVector{3, T}) where {T <: Real}
    return onsurface(a.plane, point) && zero(T) <= uv(a, point)[2] <= one(T)
end
...
function surfaceintersection(
    ...
    if interval isa EmptyInterval{T} || isinfiniteinterval(interval)
        ...
        if ρ > one(T)
            ...
            intuv =
                Intersection(α(intersect), p, normal(ell), θ, ρ, interface(ell))
...
function vertices(e::Ellipse, subdivisions::Int = 10)
    return vertices3d(e, subdivisions)[1:2, :]
end

function vertices3d(
    e::Ellipse{R},
    ::Type{Val{subdivisions}} = Val{10},
) where {R <: Real, subdivisions}
