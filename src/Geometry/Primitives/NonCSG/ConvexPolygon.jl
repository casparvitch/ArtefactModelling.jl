...
struct ConvexPolygon{N, T <: Real} <: PlanarShape{T}
    ...
    function ConvexPolygon(
        ...
        world_center =
            local2world(local_frame) *
            Vec3(local_center[1], local_center[2], zero(T))
        ...
        local_lines = SVector(
            [
                SVector(
                    ...
                    # bx = Bx - Ax,  by = By - Ay
                    SVector(
                        pts[mod(i, length(pts)) + 1][1] - pts[i][1],
                        pts[mod(i, length(pts)) + 1][2] - pts[i][2],
                    ),
...
function surfaceintersection(
    ...
    if interval isa EmptyInterval{T} || isinfiniteinterval(interval)
        ...
        @inline function orientation(
            l::SVector{3, SVector{2, T}},
        )::Int8 where {T <: Real}
