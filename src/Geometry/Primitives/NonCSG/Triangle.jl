...
struct Triangle{T} <: Surface{T}
    ...
    function Triangle(
        v1::AbstractArray{T, 1},
        v2::AbstractArray{T, 1},
        v3::AbstractArray{T, 1},
        uv1::AbstractArray{T, 1},
        uv2::AbstractArray{T, 1},
        uv3::AbstractArray{T, 1},
    ) where {T <: Real}
...
function Base.show(io::IO, a::Triangle{T}) where {T <: Real}
    return print(io, "Triangle{$T}($(a.A), $(a.B), $(a.C))")
end
...
function centroid(tri::Triangle{T}) where {T <: Real}
    return (vertex(tri, 1) + vertex(tri, 2) + vertex(tri, 3)) / 3
end
function validtri(
    v1::SVector{3, T},
    v2::SVector{3, T},
    ...
    return !any(isnan.(v1)) &&
           !any(isnan.(v2)) &&
           !any(isnan.(v3)) &&
           !samepoint(norm(cross(v2 - v1, v3 - v1)), zero(T))
end
function vertex(tri::Triangle{T}, i::Int) where {T <: Real}
    if i == 1
        tri.A
    elseif i == 2
        tri.B
    elseif i == 3
        ...
        throw(ErrorException("Invalid index: $i"))
    end
end
vertices(tri::Triangle{T}) where {T <: Real} = (tri.A, tri.B, tri.C)
function uvvertex(tri::Triangle{T}, i::Int) where {T <: Real}
    if i == 1
        tri.uv1
    elseif i == 2
        tri.uv2
...
function point(tri::Triangle{T}, a1::T, a2::T, a3::T) where {T <: Real}
    return vertex(tri, 1) * a1 + vertex(tri, 2) * a2 + vertex(tri, 3) * a3
end
function uv(tri::Triangle{T}, a1::T, a2::T, a3::T) where {T <: Real}
    return uvvertex(tri, 1) * a1 + uvvertex(tri, 2) * a2 + uvvertex(tri, 3) * a3
end
normal(tri::Triangle{T}) where {T <: Real} = tri.normal

# returns an Interval representing a half-space. If the ray and normal are 
# pointing in opposite directions then the ray is entering the surface and the 
# half-space will be Interval(surface intersection, +inf). If the ray and normal
# are pointing in the same directions then the ray is leaving the surface and 
# the half-space will be Interval(-inf,surface intersection)
...
# chuck these in here to maintain a nice heirarchy
function BoundingBoxes.BoundingBox(tri::Triangle{T}) where {T <: Real}
...
"""
function Base.:*(a::Transform{T}, t::Triangle{T}) where {T <: Real}
    return Triangle(a * vertex(t, 1), a * vertex(t, 2), a * vertex(t, 3))
end
