...
function Base.show(io::IO, hex::Hexagon{T}) where {T <: Real}
    return print(
        io,
        "Hexagon{$T}($(centroid(hex)), $(normal(hex)), $(hex.side_length), $(interface(hex)))",
    )
end
...
function makemesh(hex::Hexagon{T}, ::Int = 0) where {T <: Real}
    ...
    triangles = [
        Triangle(
            c + sin(0 * π / 3) * uvec + cos(0 * π / 3) * vvec,
            c,
            c + sin(1 * π / 3) * uvec + cos(1 * π / 3) * vvec,
        ),
        Triangle(
            c + sin(1 * π / 3) * uvec + cos(1 * π / 3) * vvec,
            c,
            c + sin(2 * π / 3) * uvec + cos(2 * π / 3) * vvec,
        ),
        Triangle(
            c + sin(2 * π / 3) * uvec + cos(2 * π / 3) * vvec,
            c,
            c + sin(3 * π / 3) * uvec + cos(3 * π / 3) * vvec,
        ),
        Triangle(
            c + sin(3 * π / 3) * uvec + cos(3 * π / 3) * vvec,
            c,
            c + sin(4 * π / 3) * uvec + cos(4 * π / 3) * vvec,
        ),
        Triangle(
            c + sin(4 * π / 3) * uvec + cos(4 * π / 3) * vvec,
            c,
            c + sin(5 * π / 3) * uvec + cos(5 * π / 3) * vvec,
        ),
        Triangle(
            c + sin(5 * π / 3) * uvec + cos(5 * π / 3) * vvec,
            c,
