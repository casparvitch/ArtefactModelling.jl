...
using Plots # FIXME
using FromFile: @from
@from "../../constants.jl" import QTYPE_PRECOMP
...
function F(m::Int, n::Int)::Float64
    ...
    if n === 0
        return Float64(
            (m^2 * factorial2(big(2m - 3))) /
            (2^(m + 1) * factorial(big(m - 1))),
        )
    elseif n > 0 && m === 1
        return Float64(
            (4(n - 1)^2 * n^2 + 1) / (8 * (2n - 1)^2) + 11 * Int(n === 1) / 32,
        )
    elseif n > 0 && m > 1
        χ = m + n - 2
        return Float64(
            (2n * χ * (3 - 5m + 4n * χ) + m^2 * (3 - m + 4n * χ)) /
            ((m + 2n - 3) * (m + 2n - 2) * (m + 2n - 1) * (2n - 1)) * γ(m, n),
        )
    else
...
# 2 eq A.14
function γ(a::Int, b::Int)
    return (factorial(big(b)) * factorial2(big(2a + 2b - 3))) /
           (2^(a + 1) * factorial(big(a + b - 3)) * factorial2(big(2b - 1)))
end
...
function G(m::Int, n::Int)::Float64
    ...
    if n === 0
        return Float64(
            factorial2(big(2m - 1)) / (2^(m + 1) * factorial(big(m - 1))),
        )
    elseif n > 0 && m === 1
        return Float64(
            -((2 * n^2 - 1) * (n^2 - 1)) / (8 * (4 * n^2 - 1)) -
            Int(n === 1) / 24,
        )
    elseif n > 0 && m > 0
        return Float64(
            -((2n * (m + n - 1) - m) * (n + 1) * (2m + 2n - 1)) /
            ((m + 2n - 2) * (m + 2n - 1) * (m + 2n) * (2n + 1)) * γ(m, n),
        )
    else
        throw(ErrorException("Invalid n and m"))
    end
...
const PRECOMP_g = Matrix{Float64}(
    reshape(
        collect(
            g(m, n, true) for n in 0:(QTYPE_PRECOMP - 1) for
            m in 1:QTYPE_PRECOMP
        ),
        (QTYPE_PRECOMP, QTYPE_PRECOMP),
    ),
)
const PRECOMP_f = Matrix{Float64}(
    reshape(
        collect(
            f(m, n, true) for n in 0:(QTYPE_PRECOMP - 1) for
            m in 1:QTYPE_PRECOMP
        ),
        (QTYPE_PRECOMP, QTYPE_PRECOMP),
    ),
)
const PRECOMP_A = Matrix{Float64}(
    reshape(
        collect(
            A(m, n, true) for n in 0:(QTYPE_PRECOMP - 1) for
            m in 1:QTYPE_PRECOMP
        ),
        (QTYPE_PRECOMP, QTYPE_PRECOMP),
    ),
)
const PRECOMP_B = Matrix{Float64}(
    reshape(
        collect(
            B(m, n, true) for n in 0:(QTYPE_PRECOMP - 1) for
            m in 1:QTYPE_PRECOMP
        ),
        (QTYPE_PRECOMP, QTYPE_PRECOMP),
    ),
)
const PRECOMP_C = Matrix{Float64}(
    reshape(
        collect(
            C(m, n, true) for n in 0:(QTYPE_PRECOMP - 1) for
            m in 1:QTYPE_PRECOMP
        ),
        (QTYPE_PRECOMP, QTYPE_PRECOMP),
    ),
...
const PRECOMP_g0 =
    Vector{Float64}(collect(g0(n, true) for n in 0:(QTYPE_PRECOMP - 1)))
const PRECOMP_f0 =
    Vector{Float64}(collect(f0(n, true) for n in 0:(QTYPE_PRECOMP - 1)))
const PRECOMP_h0 =
    Vector{Float64}(collect(h0(n, true) for n in 0:(QTYPE_PRECOMP - 1)))
...
function P(m::Int, n::Int, x::T)::T where {T <: Real}
    if m === 0
        ...
        elseif n === 1
            ...
            # 2 eq A.2
            return (A(m, n - 1) + B(m, n - 1) * x) * P(m, n - 1, x) -
                   C(m, n - 1) * P(m, n - 2, x)
        end
    end
end
...
function plot!(m::Int, n::Int)
    ...
    return Plots.plot!(us, vs, label = if m === 0
        "\$u^2(1-u^2)Q^{$m}_{$n}(u^2)\$"
    else
        "\$u^mQ^{$m}_{$n}(u^2)\$"
    end)
end
...
struct QTypeSurface{T, D, M, N} <: ParametricSurface{T, D}
    ...
    function QTypeSurface(
        semidiameter::T;
        radius::T = typemax(T),
        conic::T = zero(T),
        αcoeffs::Union{Nothing, Vector{Tuple{Int, Int, T}}} = nothing,
        βcoeffs::Union{Nothing, Vector{Tuple{Int, Int, T}}} = nothing,
        ...
        NP1 = N + 1
        return new{T, 3, M, NP1}(
            semidiameter,
            1 / radius,
            conic,
            Cylinder(semidiameter, interface = opaqueinterface(T)),
            SVector{NP1, T}(b0coeffs),
            SVector{M, SVector{NP1, T}}(dαcoeffs),
            SVector{M, SVector{NP1, T}}(dβcoeffs),
            normradius,
            maxheight,
...
function uvrange(::Type{QTypeSurface{T, D, M, N}}) where {T <: Real, D, M, N}
...
function point(
    z::QTypeSurface{T, 3, M, N},
    ρ::T,
    θ::T,
)::SVector{3, T} where {T <: Real, M, N}
    rad = z.semidiameter
    r = ρ * rad
    # ρ is normalised [0, 1]
    # r is absolute
    ...
    if N > 0
        ...
        @inbounds @simd for m in 1:M
            tot +=
                u^m * (
                    cos(m * θ) * QType.S(z.dαcoeffs[m], m, u2)::T +
                    sin(m * θ) * QType.S(z.dβcoeffs[m], m, u2)::T
                )
        end
        h += (sqrt(p) / sqrtq) * tot
    end
...
function partials(
    ...
    ρ::T,
    θ::T,
)::Tuple{SVector{3, T}, SVector{3, T}} where {T <: Real, M, N}
    rad = z.semidiameter
    r = ρ * rad
    # ρ is normalised [0, 1]
    # r is absolute
    ...
    if N > 0
        ...
        dbdu =
            2 * u * (one(T) - 2 * u2) * S0v +
            2 * u^3 * (one(T) - u2) * QType.dS0dx(z.b0coeffs, u2)::T
        c = zero(T)
        dcdu = zero(T)
        if M > 0
            @inbounds @simd for m in 1:M
                ...
                dcdu +=
                    u^(m - 1) * (
                        ...
                        u2 *
                        (
                            cosmθ * QType.dSdx(z.dαcoeffs[m], m, u2)::T +
                            sinmθ * QType.dSdx(z.dβcoeffs[m], m, u2)::T
                        )
                    )
            end
        end
...
function normal(
    z::QTypeSurface{T, D, M, N},
    ρ::T,
    θ::T,
)::SVector{3, T} where {T <: Real, D, M, N}
...
function surfaceintersection(
    surf::AcceleratedParametricSurface{T, 3, QTypeSurface{T, 3, M, N}},
    r::AbstractRay{T, 3},
) where {T <: Real, M, N}
...
) where {T <: Real, N, S <: QTypeSurface{T, N}}
...
function BoundingBox(surf::QTypeSurface{T, 3}) where {T <: Real}
    ...
    if q < zero(T)
        throw(
            ErrorException(
                "The surface is invalid, no bounding box can be constructed",
            ),
        )
    end
