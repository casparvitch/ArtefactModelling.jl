...
```
"""
struct AcceleratedParametricSurface{T, N, S <: ParametricSurface{T, N}} <:
    ...
    ) where {T <: Real, N, S <: ParametricSurface{T, N}}
        a = AcceleratedParametricSurface(
            surf,
            triangulate(surf, numsamples, true, true),
            interface = interface,
        )
        emptytrianglepool!(T)
        return a
...
function uvrange(
    ::Type{AcceleratedParametricSurface{T, N, S}},
) where {T <: Real, N, S <: ParametricSurface{T, N}}
    return uvrange(S)
end
function uvrange(
    surf::AcceleratedParametricSurface{T, N, S},
) where {T <: Real, N, S <: ParametricSurface{T, N}}
    return uvrange(surf.surface)
end
function point(
    surf::AcceleratedParametricSurface{T, N, S},
    u::T,
...
function partials(
    ...
    return partials(surf.surface, u, v)
end
function normal(
    surf::AcceleratedParametricSurface{T, N, S},
    u::T,
    ...
    return normal(surf.surface, u, v)
end
function inside(
    surf::AcceleratedParametricSurface{T, N, S},
    x::T,
    ...
    return inside(surf.surface, x, y, z)
end
function onsurface(
    surf::AcceleratedParametricSurface{T, N, S},
    x::T,
    ...
    return onsurface(surf.surface, x, y, z)
end
"""
...
function uv(
    surf::AcceleratedParametricSurface{T, 3},
    x::T,
    y::T,
    z::T,
) where {T <: Real}
    return uv(surf.surface, x, y, z)
end
...
function surfaceintersection(
    surf::AcceleratedParametricSurface{T, N},
    r::AbstractRay{T, N},
) where {T, N}
...
"""
function triangulatedintersection(
    surf::AcceleratedParametricSurface{T, N, S},
    r::AbstractRay{T, N},
) where {T <: Real, N, S}
    result = newinintervalpool!(T)
    offset_ray =
        Ray(origin(r) - ACCEL_SURF_RAY_OFFSET * direction(r), direction(r))
    ...
    @inbounds for i in 2:nr
        ...
        j = i - 1
        while j > 0 &&
            α(halfspaceintersection(result[j])) >
            α(halfspaceintersection(value))
            result[j + 1] = result[j]
            j = j - 1
        end
        result[j + 1] = value
...
function jacobian(
    ...
    return SMatrix{2, 2, T}(
        dot(view(P1, 1:(M - 1)), du),
        dot(view(P2, 1:(M - 1)), du),
        dot(view(P1, 1:(M - 1)), dv),
        dot(view(P2, 1:(M - 1)), dv),
    )
end
...
function Base.show(
    io::IO,
    a::AcceleratedParametricSurface{S, N, T},
) where {S <: Real, N, T <: ParametricSurface{S, N}}
    return println(io, "Accelerated$(string(a.surface))")
end
